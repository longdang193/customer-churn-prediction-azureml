...
function Show-Info {
    ...
    if (-not [string]::IsNullOrEmpty($AcrName)) {
        Write-Host "2. Update aml/environments/environment.yml with ACR name: $AcrName"
        Write-Host "3. Build and push Docker image to ACR (see Step 11 in MASTER_PLAN.md)"
    }
    else {
